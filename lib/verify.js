...
export default async function verify(pluginConfig, context, { Octokit }) {
  ...
  if (
    ...
    // compare only the <owner>/<repo> part of the two URLs in case one URL is HTTPS
    // and the other is SSH
    const getRepoPath = url => new URL(url).pathname.replace(/\.git$/, '')
    try {
      if (status !== 200 || getRepoPath(repositoryUrl) !== getRepoPath(clone_url)) {
