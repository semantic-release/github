...
export default async function verify(pluginConfig, context, { Octokit }) {
  ...
  if (
    ...
    !errors.find(({ code }) => code === "EINVALIDGHTOKEN")
  ) {
    const octokit = new Octokit(
      toOctokitOptions({
        githubToken,
    ...
    const git_url = data?.git_url;

    if (status !== 200) {
      errors.push(getError("EMISMATCHGITHUBURL", { repositoryUrl, git_url }));
    } else {
      const parsedRepositoryUrl = parseGithubUrl(repositoryUrl);
      const parsedGitUrl = parseGithubUrl(git_url);

      if (
        parsedRepositoryUrl.owner !== parsedGitUrl.owner ||
        parsedRepositoryUrl.repo !== parsedGitUrl.repo
      ) {
        errors.push(getError("EMISMATCHGITHUBURL", { repositoryUrl, git_url }));
      }
    }
