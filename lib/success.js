...
module.exports = async (pluginConfig, context) => {
  ...
  if (successComment === false) {
    ...
    const prs = await pFilter(uniqBy(flatten(await Promise.all(searchQueries)), 'number'), async ({number}) => {
      const options = {owner, repo, pull_number: number}; // eslint-disable-line camelcase

      return (
        (await github.pullRequests.listCommits(options)).data.find(({sha}) => shas.includes(sha)) ||
        shas.includes((await github.pullRequests.get(options)).data.merge_commit_sha)
      );
    });
    ...
    await Promise.all(
      uniqBy([...prs, ...issues], 'number').map(async issue => {
        ...
        try {
          const state = issue.state || (await github.issues.get({owner, repo, issue_number: issue.number})).data.state; // eslint-disable-line camelcase

          if (state === 'closed') {
            const comment = {owner, repo, issue_number: issue.number, body}; // eslint-disable-line camelcase
  ...
  if (failComment === false || failTitle === false) {
    ...
    await Promise.all(
      srIssues.map(async issue => {
        ...
        try {
          const updateIssue = {owner, repo, issue_number: issue.number, state: 'closed'}; // eslint-disable-line camelcase
