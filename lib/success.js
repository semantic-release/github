...
export default async function success(pluginConfig, context, { Octokit }) {
  ...
  if (successComment === false) {
    ...
    const { repository } = await octokit.graphql(buildAssociatedPRsQuery(shas), { owner, repo });
    const associatedPRs = Object.values(repository).map((item) => item.associatedPullRequests.nodes);
