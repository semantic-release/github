  ...
  if (successComment === false || isEmpty(commits)) {
    ...
    for (const chunk of shasChunks) {
      ...
      for (const { nodes, pageInfo } of responseAssociatedPRs) {
        associatedPRs.push(...buildIssuesOrPRsFromResponseNode(nodes, "PR"));
        if (pageInfo.hasNextPage) {
          ...
          while (hasNextPage) {
            ...
            associatedPRs.push(
              ...buildIssuesOrPRsFromResponseNode(associatedPullRequests.nodes, "PR"),
            );
    ...
    const { repository } = await octokit.graphql(
      buildRelatedIssuesQuery(
        uniqBy(flatten(parsedIssues), "number").map((issue) => issue.number),
      ),
      { owner, repo },
    );
    ...
    const issues = buildIssuesOrPRsFromResponseNode(responseRelatedIssues);

    debug("found related issues via PRs and Commits: %O", issues);
    ...
    await Promise.all(
      uniqBy([...prs, ...issues], "number").map(async (issue) => {
        ...
        try {
          ...
          logger.log(`Added comment to ${issueOrPR} #%d: %s`, issue.number, url);
          ...
          if (releasedLabels) {
            ...
            logger.log(`Added labels %O to ${issueOrPR} #%d`, labels, issue.number);
          }
        } catch (error) {
          if (error.status === 403) {
            logger.error(
              `Not allowed to add a comment to the ${issueOrPR} #%d.`,
              issue.number,
            );
          } else if (error.status === 404) {
            logger.error(
              `Failed to add a comment to the ${issueOrPR} #%d as it doesn't exist.`,
              issue.number,
            );
          } else {
            errors.push(error);
            logger.error(
              `Failed to add a comment to the ${issueOrPR} #%d.`,
              issue.number,
            );
 ...
 * @returns {object[]}
 */
function buildIssuesOrPRsFromResponseNode(responseNodes, type) {
  const resultArray = [];
