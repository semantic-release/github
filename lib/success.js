  ...
  if (successComment === false || isEmpty(commits)) {
    ...
    for (const chunk of shasChunks) {
      ...
      for (const { nodes, pageInfo } of responseAssociatedPRs) {
        associatedPRs.push(...buildAssociatedPRs(nodes));
        if (pageInfo.hasNextPage) {
          ...
          while (hasNextPage) {
            ...
            associatedPRs.push(
              ...buildAssociatedPRs(associatedPullRequests.nodes),
            );
    ...
    debug("found issues via comments: %O", parsedIssues);
 ...
 * @param {object} associatedPRsResponseNodes
 * @returns {object[]}
 */
function buildAssociatedPRs(associatedPRsResponseNodes) {
  const associatedPRs = [];
  for (const node of associatedPRsResponseNodes) {
    const pr = {
      pull_request: true,
      number: node.number,
      title: node.title,
