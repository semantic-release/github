  ...
  if (failComment === false || failTitle === false) {
    ...
    if (srIssue) {
      ...
      try {
        ...
        createdIssue = await octokit.request(
          "POST /repos/{owner}/{repo}/issues",
          newIssue,
        );
      } catch (error) {
        const hasValidationFailure = error.status === 422;

        if (!hasValidationFailure) throw error;
        ...
        debug(
          "issue create failed due to label validation; retrying without labels: %O",
          issueWithoutLabels,
        );
        createdIssue = await octokit.request(
          "POST /repos/{owner}/{repo}/issues",
